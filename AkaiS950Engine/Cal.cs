...
namespace AkaiS950Engine
{
    ...
    public static class Cal
    {
        ...
        ///
        /// There used to be nothing measured between stored 0 and 50 - a fifty-byte hole
        /// holding two in five of the VCA releases on the real disks, filled by
        /// interpolating geometrically between the ends. Run 19 walked a release ladder and
        /// a decay ladder across it; they agree rung for rung to 1.5%, so EnvTime is one
        ...
        ///
        /// IT IS NOT SMOOTH, WHICH IS WHY NO INTERPOLATION COULD HAVE FOUND IT. Ten stored
        /// units double the time, seven times over - x2.079 to x2.185 - but the two
        /// five-unit steps inside each decade are nothing like equal halves of that:
        ///
        ///     15->20 x1.613   20->25 x1.321      35->40 x1.607   40->45 x1.356
        ///     25->30 x1.574   30->35 x1.360      45->50 x1.574   50->55 x1.338
        ///
        /// An even split would be x1.463 twice. It alternates 1.60 then 1.34, four times
        /// each, on both ladders independently - the same counter signature the VCA attack
        /// turned out to have.
        ///
        /// THE STORED-50 ANCHOR WAS THE ONE THAT WAS WRONG. Every value here is a measured
        /// RATE in dB per second turned into a time by VcaReleaseDb. Run 9's anchors are
        /// times, from tracking the filter's corner, so they are frequencies and the
        /// limiter that spoilt its levels left them alone - and two of three agree with run
        /// 19 on the span: stored 55 implies 43.3 dB and stored 65 implies 42.4, against
        /// stored 50's 49.3. So 50 was 16% slow, everything interpolated below it inherited
        /// that, and the 20% error found around stored 45 during the crossfade work was the
        /// edge of it.
        ///
        /// 0 to 15 is still interpolated, across a stretch now known to step rather than
        /// glide. The fall at stored 15 is over in nine milliseconds, as short as an rms
        /// window can time.
        ...
        ///
        /// Run 20 tried and could not: it watched the filter's RELEASE, which only happens
        /// after the key comes up - and the amplitude is released at the same moment, so
        /// the note was dying at 15 dB a second underneath the measurement. The probes
        /// inside each clip disagreed by up to 3.4x, growing with the setting, which is
        /// that fault rather than the filter.
        ///
        /// Run 21 watched the filter's DECAY instead. A decay happens while the key is
        /// still down, so the amplitude could be held dead flat and the corner followed
        /// against a level that does not move. Six settings, each read against the same
        /// disk rendered through this model so the measurement's own lag cancels:
        ///
        ...
        public const double VcfTimeScale = 0.71;
        ...
        ///
        /// THE FOUR THAT MISSED WERE THE CURVE, NOT THE SPAN - AND RUN 19 FOUND THEM. The
        /// clips near stored 45 implied 49.3 dB where the rest said 41, and this note used
        /// to say that meant EnvTime was 20% slow there and wanted re-measuring. It did,
        /// and it was: the table's anchor at stored 50 was 16% slow, so everything
        /// interpolated below it was wrong and clips landing there could not agree with
        /// clips landing anywhere else. See EnvTime.
        ///
        /// 40 -> 42.5 comes from the two anchors that survived. Run 9's times were measured
        /// by tracking the filter's corner, so they are frequencies and the limiter that
        /// spoilt its levels left them alone; run 19's rates are clean. Multiply them and
        /// stored 55 implies 43.3 dB, stored 65 implies 42.4, against stored 50's 49.3.
        ///
        /// This and EnvTime move together and their product does not: every rate run 19
        /// measured is reproduced, and settings it did not reach improve too - at stored 65
        /// the model now gives 48.3 dB/s where run 19 read 48.2, against 45.4 before.
        ///
        /// It is a RATE, so this is how far the level falls in one release time from
        /// wherever the key came up - not the distance it has to cover before it stops. The
        /// DECAY reads the same curve and the same span: run 19 measured both ladders at
        /// eight settings and they agree to 1.5%, so there is one number here, not two.
        /// </summary>
        ...
        /// 5, and a stored 0 is NOT on it. Run 19 traced one: it falls at the same rate as
        /// every other sustain setting, straight past where the line would stop it, hovers
        /// a moment in the 12-bit quantisation around -60 dB, then goes to the noise floor
        /// and stays. Between 1.873 s and 1.982 s at 48.2 dB/s, so 90 to 96 dB down.
        ///
        /// 96.0 dB is 240 steps of 0.4 exactly - the machine's own decibel step, the one
        /// the crossfade turned out to count in - and it sits inside that bracket.
        ///
        /// 1907 of the 1908 library keygroups set a decay and 723 decay to a sustain of 20
        /// or less, so every plucked and struck sound on every disk is one of these. They
        /// used to stop dead 39.6 dB down and sit there ringing.
        /// </summary>
        public const double VcaSilenceDb = 96.0;

        /// <summary>
        /// How far below full level a stored sustain rests. A stored 0 is silence rather
        /// than the bottom of the line the rest sit on - see VcaSilenceDb.
        /// </summary>
        public static double SustainDbFor(double stored)
        {
            double s = Clamp(stored, 0, 99);
            return s <= 0 ? -VcaSilenceDb : -(1.0 - s / 99.0) * SustainDb;
        }

        /// <summary>
        /// THE AMPLITUDE DECAY IS A RATE, NOT A DURATION, so how long it takes depends on
        /// how far it has to go.
        ///
        /// Run 19 held the decay at stored 65 and moved the sustain through twelve settings
        /// from 0 to 99. Every one fell at 48.2 dB per second - 0.415 s to lose twenty
        /// decibels at one end of the ladder against 0.414 at the other. A duration would
        /// have put every plateau at the same moment whatever its depth; a rate gets the
        /// shallow ones there sooner, and that is what the machine does.
        ///
        /// The FILTER's decay is still modelled as a duration. That is where this rule came
        /// from in the first place and it has never been measured.
        /// </summary>
        public static double VcaDecaySeconds(double stored, double sustainDb)
        {
        ...
        /// `sustain` is the envelope's resting fraction, 0 to 1, so 1 - sustain is the share
        /// of the full depth this decay has to cover.
        ///
        /// Run 19 found this for the amplitude. The filter's was left as a DURATION because
        /// that is how it was written before anybody measured either, and run 21 asked it
        /// properly: one decay setting, four depths, with the amplitude held dead flat so
        /// the corner could be followed against a level that does not move.
        ///
        ///     octaves travelled   1.97   1.34   0.65
        ///     seconds             0.82   0.66   0.51
        ///
        /// A duration is a flat line through those. They fit time = 0.354 + 0.235 x octaves
        /// to within 0.008 s, and the same disk rendered through the old model - which WAS a
        /// duration - comes back with a slope of -0.038. The intercept is the method's own
        /// lag; the slope is the filter.
        ///
        ...
        /// </summary>
        public static double VcfDecaySeconds(double stored, double sustain)
        {
            return EnvSeconds(stored) * VcfTimeScale * (1.0 - Clamp(sustain, 0, 1));
        }
        ...
        ///
        /// 6.25 was chosen because it is a sixteenth of a semitone exactly, and the fit
        /// behind it gave 6.21 and could not separate 6.0 from 6.5. What that fit never did
        /// was vary the DEPTH: runs 10, 11 and 12 all pinned byte 13 at -50 and swept byte
        /// 12 and velocity instead, so the depth constant came out sideways from clips
        ...
        ///
        /// A line through the origin gives 6.436 cents per unit, residual rms 12 cents,
        /// standard error 0.114. So 6.0 is 3.8 standard errors away and dead; 6.25 is 1.6
        /// away and no longer the best estimate; 6.5 is 0.6 away and would also fit.
        ///
        /// Not the limiter, before anyone assumes it: clipping a sine does not move its
        /// zero crossings and the pitch is read by counting them, so runs 10 to 12 being
        /// limited would barely have touched this. Nobody had varied the byte.
        public const double WarpCentsPerUnit = 6.44;
        ...
        /// clear of the floor. Two keys sounding one keygroup alone, four velocity spans:
        ///
        ///     velocity      1      32      64      96
        ///     dB down    32.7    24.6    16.4     8.0
        ///     per step  0.6423  0.6409  0.6443  0.6387
        ///
        /// 0.642, and dead linear - 0.9% across a 33 dB slide. The old 0.63 came from a
        /// depth of 99, whose bottom two velocities sat at and under the noise floor of a
        /// take that was being limited anyway.
        public const double VelDbPerStep = 0.642;
